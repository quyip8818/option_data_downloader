...
def process_option(df_raw, current_price, is_call, today):
    ...
    df2 = df2.dropna(
        subset=["contractSymbol", "lastTradeDate", "strike", "lastPrice", "bid", "ask"]
    )
    df2.fillna(0)
    df2["lastTradeDate"] = (
        df2["lastTradeDate"].dt.tz_convert("EST").dt.tz_localize(None)
    )

    df = df2.copy()
    df = df[(df["bid"] > 0) & (df["ask"] > 0) & (df["lastPrice"] > 0)]
    df["estPrice"] = df.apply(
        lambda row: get_est_price(
            row["ask"], row["bid"], row["lastPrice"], row["lastTradeDate"], today
        ),
        axis=1,
    )
    df["ba_spread"] = df.apply(
        lambda row: get_ba_spread(row["bid"], row["ask"]), axis=1
    )
    df["otm_r"] = df.apply(
        lambda row: get_otm_ratio(current_price, row["strike"], is_call), axis=1
    )

    df["inMoney"] = (
        (current_price - df["strike"]).clip(lower=0)
        if is_call
        else (df["strike"] - current_price).clip(lower=0)
    )
    ...
    df = (
        pd.concat([df, df2_filtered], ignore_index=True)
...
def process_max_time_value_df(df_raw, current_price):
    ...
    df["yearValuePercent"] = np.where(
        df["days"] < 7, df["dayValuePercent"] * 252, df["dayValuePercent"] * 360
    )
    ...
    payback_weeks = (
        max_day_row["value"] / week_row["value"]
        if max_day_row["value"].any() and week_row["value"].any()
        else ""
    )
    ...
    iv_ratio = (
        long_term_min_iv / week_row["impliedVolatility"]
        if long_term_min_iv.any() and week_row["impliedVolatility"].any()
        else ""
    )

    volume_ratio = (
        max_day_row["volume"] / week_row["volume"]
        if max_day_row["volume"].any() and week_row["volume"].any()
        else ""
    )

    open_interest_ratio = (
        max_day_row["openInterest"] / week_row["openInterest"]
        if max_day_row["openInterest"].any() and week_row["openInterest"].any()
        else ""
    )
    ...
    return (
        ...
        [
            round_num(iv_ratio, 3),
            round_num(week_row["impliedVolatility"], 3),
            round_num(long_term_min_iv, 3),
            round_num(max_day_row["impliedVolatility"], 3),
        ],
        [round_num(volume_ratio, 2), week_row["volume"], max_day_row["volume"]],
        [
            round_num(open_interest_ratio, 2),
            week_row["openInterest"],
            max_day_row["openInterest"],
        ],
...
def process_header_data(writer, sheet_name, data):
    pd.DataFrame.from_dict(data, orient="index").to_excel(
        writer, sheet_name=sheet_name, index=True, header=False, startrow=0
    )
    return len(data) + 1
...
def process_option_data(symbol, folder, file_name, today):
    ...
    if earnings_dates is None or earnings_dates.empty:
        ...
        try:
            next_earnings_date = (
                earnings_dates[earnings_dates.index > today_pd]
                .index.min()
                .to_pydatetime()
            )
        except:
    ...
    call_dfs = []
    put_dfs = []
    call_max_time_value_df = pd.DataFrame(
        columns=[
            "days",
            "value",
            "impliedVolatility",
    ...
    put_max_time_value_df = pd.DataFrame(
        columns=[
            "days",
            "value",
            "impliedVolatility",
    ...
    for date_str in ticker.options:
        ...
        call_max_time_value_df.loc[len(call_max_time_value_df)] = [
            diff_days,
            *call_max_time_value,
        ]

        put_max_time_value = get_max_time_value(put_df, current_price)
        put_max_time_value_df.loc[len(put_max_time_value_df)] = [
            diff_days,
            *put_max_time_value,
        ]

    (
        call_max_time_value_df,
        call_paybacks,
        call_ivs,
        call_volumes,
        call_open_interest,
    ...
    with pd.ExcelWriter(f"{folder}/{file_name}.xlsx") as writer:
        next_start_row = process_header_data(
            writer,
            "c_all",
            {
                "today": [today_str],
        ...
        call_max_time_value_df.to_excel(
            writer, sheet_name="c_all", index=False, startrow=next_start_row
        )
        ...
        put_max_time_value_df.to_excel(
            writer, sheet_name="p_all", index=False, startrow=next_start_row
        )
        ...
        for call_df in call_dfs:
            current_price_df.to_excel(
                writer, sheet_name=call_df[1], index=False, header=False, startrow=0
            )
            call_df[0].to_excel(writer, sheet_name=call_df[1], index=False, startrow=2)
        for put_df in put_dfs:
            current_price_df.to_excel(
                writer, sheet_name=put_df[1], index=False, header=False, startrow=0
            )
