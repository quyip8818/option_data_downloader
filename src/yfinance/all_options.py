...
def decode_iv_rank(iv_rank_df, symbol):
    ...
    return (
        (
            iv_rank["ivcall10"],
            iv_rank["ivcall10_rank"],
            iv_rank["ivmean10"],
            iv_rank["ivmean10_rank"],
            iv_rank["ivcall1080"],
            iv_rank["ivcall1080_rank"],
        ),
        (
            iv_rank["ivput10"],
            iv_rank["ivput10_rank"],
            iv_rank["ivmean10"],
            iv_rank["ivmean10_rank"],
            iv_rank["ivput1080"],
            iv_rank["ivput1080_rank"],
        ),
    )
...
def fetch_all_yf_options(today, iv_rank_df, skip_symbols):
    ...
    with open(file_name, "a", newline="", encoding="utf-8") as csvfile, open(
        file_error_name, "a", newline="", encoding="utf-8"
    ) as errorfile:
        ...
        for idx, symbol in enumerate(symbols):
            ...
            processed_data = process_option_data(
                symbol, folder, f"{symbol}_{today_str}", today
            )
            ...
            next_earnings_days = (
                ...
                else (next_earnings_date.date() - today).days
            )
            ...
            try:
                if (
                    float(call_ivs[0]) <= 0.7
                    and float(put_ivs[0]) <= 0.7
                    and float(call_ivs[2]) <= 0.3
                    and float(put_ivs[2]) <= 0.3
                    and float(call_bid_ask_diff[1]) <= 0.6
                    and float(put_bid_ask_diff[1]) <= 0.6
                ):
                    print(summary_row)
            except:
                pass
